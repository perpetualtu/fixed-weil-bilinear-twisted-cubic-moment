...
\paragraph{Dependency note (logical order).}
The collision-removability result \Cref{prop:P-holomorphic}, although placed
with the collision discussion in \Cref{sec:final}, is an algebraic input to
the next proposition, not a consequence of it.  More precisely, its proof
uses only the explicit three-term definition \eqref{eq:P-lambda}, the Euler
product and archimedean identities \eqref{eq:Z-euler} and \eqref{eq:X-def},
and the local division statement \Cref{lem:three-divisor}.  It uses neither
\Cref{lem:two-constants} nor \Cref{prop:full-kernel-continuation}, and it uses
no polynomial majorant or contour move proved below.  Thus the logical
dependency chain is
\[
 \begin{gathered}
 \eqref{eq:P-lambda},\ \eqref{eq:Z-euler},\ \eqref{eq:X-def}\\
 \Downarrow\\
 \Cref{lem:three-divisor}\\
 \Downarrow\\
 \Cref{prop:P-holomorphic}\\
 \Downarrow\\
