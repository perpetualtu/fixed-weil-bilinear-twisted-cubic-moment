...
Throughout this section the convention following \eqref{eq:w-derivatives}
is in force.  Thus the constants behind $T^\eps$ may depend on the fixed
$C,B_0,M$, on finitely many seminorms
\eqref{eq:w-seminorm}--\eqref{eq:auxiliary-weight-seminorm}, and on the fixed
completion order, but not on the dyadic block parameters or on $H,K,T$.
The parameter $\eta$ enters only when \eqref{eq:main-range} is invoked in the
global summation.  The auxiliary $\eps_0$ used below is chosen only after the
completion order is known and is then absorbed into the final $\eps$; it is
not an additional uniformity parameter.
