...
\begin{equation}\label{eq:reciprocal-sum}
 ...
 \ll_\eps Zq^\eps
 \left(Xq^{-1/2}+q^{1/2}\right)(c,q)^{1/2}.
\end{equation}
Moreover,
...
\]
The Weil bound
$|S(c,n;q)|\leq\tau(q)(c,n,q)^{1/2}q^{1/2}$ therefore proves
\eqref{eq:reciprocal-sum}, uniformly for both $X\leq q$ and $X>q$; the
term $n=0$ is the Ramanujan sum and obeys the same majorant.  Finally,
...
\begin{align}
 ...
 &\qquad\ll T^\eps
 \left(\frac{M_0g}{d}q^{-1/2}+q^{1/2}\right)(a\rho,q)^{1/2}.
 \label{eq:m-reciprocal}
\end{align}
Put $Q_d=H_0/d$.  By \eqref{eq:gcd-average}, restriction to
$(q,k_1)=1$ can only decrease the majorant, and
\begin{align}
 &\sum_{q\asymp Q_d}
