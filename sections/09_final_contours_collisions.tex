...
The earlier passage from $\Re u=1$ to $c_u$ in
\eqref{eq:zeta-star} is instead the pole-free finite-truncation move of the
inverse-Mellin kernel described after \eqref{eq:cu-range}; it is not a contour
move of this collapsed $X$-factor.  Numerator-gamma poles crossed during the
later branchwise continuation are the remote $u$-residues
\eqref{eq:remote-u-poles}, already recorded there as exponentially small.
Denominator gamma factors give zeros.
\end{enumerate}
...
\end{align*}
 ...
 \eqref{eq:X-stirling}, with its total uniform error, follows from
 \Cref{prop:collision-safe-stirling}; in particular, no individual
 meromorphic $Z$-term is estimated at a collision.  This completes the proof.
